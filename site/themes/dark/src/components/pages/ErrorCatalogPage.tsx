...
export function ErrorCatalogPage({ data }: ErrorCatalogPageProps) {
  ...
  return (
    <div>
      <Breadcrumb items={[{ label: "Architecture", href: "/architecture" }, { label: "Error Catalog" }]} />

      <h1 className="text-2xl font-bold text-text-primary mb-2">Error Catalog</h1>
      <p className="text-text-secondary mb-6">All documented errors from {artifact.label}</p>
      ...
      <input
        ...
        className="w-full px-4 py-2 mb-6 rounded-lg border border-border bg-surface-secondary text-sm text-text-primary placeholder:text-text-tertiary focus:outline-none focus:ring-2 focus:ring-primary-500"
      />
      ...
      <div className="space-y-4">
        {filtered.map((error) => (
          <div key={error.code} className="p-4 rounded-lg border border-border" id={`error-${error.code}`}>
            ...
            {error.description && <p className="text-sm text-text-secondary mb-3">{error.description}</p>}
            ...
            <div className="flex flex-wrap gap-4 text-xs text-text-tertiary">
              {error.thrownBy && error.thrownBy.length > 0 && <span>Thrown by: {error.thrownBy.join(", ")}</span>}
              {error.endpoints && error.endpoints.length > 0 && <span>Endpoints: {error.endpoints.join(", ")}</span>}
