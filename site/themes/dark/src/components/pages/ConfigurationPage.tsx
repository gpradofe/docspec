...
export function ConfigurationPage({ data }: ConfigurationPageProps) {
  ...
  return (
    <div>
      <Breadcrumb items={[{ label: "Architecture", href: "/architecture" }, { label: artifact.label }, { label: "Configuration" }]} />

      <h1 className="text-2xl font-bold text-text-primary mb-2">Configuration</h1>
      <p className="text-text-secondary mb-2">Configuration properties for {artifact.label}.</p>
      ...
      {groups.map(([source, props]) => (
        <section key={source} className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            <h2 className="text-lg font-semibold text-text-primary" id={`source-${slugify(source)}`}>{source}</h2>
...
function PropertyRow({ property }: { property: ConfigurationProperty }) {
  return (
    <tr className="border-b border-border/50">
      <td className="py-2 pr-4"><code className="font-mono text-sm text-text-primary">{property.key}</code></td>
      <td className="py-2 pr-4">{property.type ? <code className="font-mono text-xs text-text-secondary">{property.type}</code> : <span className="text-text-tertiary">{"\u2014"}</span>}</td>
      <td className="py-2 pr-4">{property.default ? <code className="font-mono text-xs text-text-secondary">{property.default}</code> : <span className="text-text-tertiary">{"\u2014"}</span>}</td>
      <td className="py-2 pr-4">{property.environment ? <Badge variant="primary">{property.environment}</Badge> : <span className="text-text-tertiary">{"\u2014"}</span>}</td>
      <td className="py-2">
        <div>
          ...
          {property.validRange && (
            <div className="mt-1 text-xs text-text-tertiary">
              {property.validRange.min !== undefined && property.validRange.max !== undefined && (
                <span>Range: {property.validRange.min}&ndash;{property.validRange.max}</span>
              )}
            </div>
          )}
          ...
          {property.usedBy && property.usedBy.length > 0 && (
            <div className="mt-1 flex flex-wrap gap-1">
              {property.usedBy.map((u) => <Badge key={u}>{u}</Badge>)}
            </div>
          )}
        </div>
      </td>
