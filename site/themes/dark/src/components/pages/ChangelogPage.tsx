...
const STATUS_COLORS = {
  added: { bg: "rgba(34,197,94,0.15)", text: "#4ade80", label: "Added" },
  removed: { bg: "rgba(239,68,68,0.15)", text: "#f87171", label: "Removed" },
  modified: { bg: "rgba(96,165,250,0.15)", text: "#60a5fa", label: "Modified" },
};
    ...
    <div>
      ...
      {diffs.length === 0 ? (
        <p className="text-text-tertiary text-center py-12">
          No version history available. Run <code className="font-mono bg-surface-tertiary px-1.5 py-0.5 rounded">docspec diff</code> to compare versions.
        </p>
      ) : (
        <div
          style={{
            position: "relative",
            paddingLeft: 24,
            borderLeft: "2px solid var(--ds-border, #334155)",
          }}
        >
          {diffs.map((diff, i) => (
            <div key={i} style={{ marginBottom: 40, position: "relative" }}>
              <div
                style={{
                  ...
                  height: 12,
                  borderRadius: "50%",
                  background: "var(--ds-primary, #818cf8)",
                  border: "2px solid var(--ds-surface-primary, #0f172a)",
                }}
              />
...
function ChangeEntry({ name, status, detail, kind }: { name: string; status: string; detail?: string; kind: string }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px", borderRadius: 6, border: "1px solid var(--ds-border, #334155)" }}>
