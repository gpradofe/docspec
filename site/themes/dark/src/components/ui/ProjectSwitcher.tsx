...
export function ProjectSwitcher({ projects, current }: ProjectSwitcherProps) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 w-48 bg-surface-secondary border border-border rounded-lg shadow-lg py-1 z-50">
          {projects.map((project) => (
            <a
              ...
              className={`flex items-center gap-2 px-3 py-2 text-sm hover:bg-surface-tertiary ${
                project.slug === current ? "text-primary-500 font-medium" : "text-text-secondary"
              }`}
            >
              {project.color && (
                <span
