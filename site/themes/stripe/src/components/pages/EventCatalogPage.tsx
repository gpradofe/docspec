          ...
          <div key={event.name} className="p-4 rounded-lg border border-border">
            <div className="flex items-center gap-3 mb-2">
              ...
              {event.deliveryGuarantee && (
                <Badge variant="info">{event.deliveryGuarantee}</Badge>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm mb-3">
              ...
              {event.channel && (
                <div>
                  ...
                  <code className="font-mono text-text-secondary">{event.channel}</code>
                </div>
              )}
              {event.retryPolicy && (
                <div>
                  <span className="text-text-tertiary">Retry: </span>
                  <span className="text-text-secondary">{event.retryPolicy}</span>
                </div>
              )}
              {event.since && (
                <div>
                  <span className="text-text-tertiary">Since: </span>
                  <span className="text-text-secondary">{event.since}</span>
                </div>
              )}
            </div>
                  ...
                  <tbody>
                    {event.payload.fields.map((f) => (
                      <tr key={f.name} className="border-b border-border/50">
                        ...
                        <td className="py-1 pr-3 font-mono text-primary-600">{f.type}</td>
