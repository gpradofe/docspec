...
export function MethodSignature({ method, className = "" }: MethodSignatureProps) {
  ...
  return (
    <code className={`text-sm font-mono ${className}`}>
      <span className="text-purple-600">{visibility}</span>
      {modifiers && <span className="text-blue-600"> {modifiers}</span>}
      {" "}
      <span className="text-emerald-600">{returnType}</span>
      {" "}
      ...
      {params.map((p, i) => (
        <React.Fragment key={p.name}>
          ...
          <span className="text-emerald-600">{p.type.replace(/java\.lang\./g, "").replace(/java\.util\./g, "")}</span>
          {" "}
