  ...
  return (
    <div className="rounded-lg border border-border overflow-hidden my-4">
      <div className="px-4 py-2 border-b border-border bg-surface-secondary">
        <h4 className="text-xs font-semibold text-text-tertiary uppercase">JSON Shape</h4>
        {jsonShape.description && <p className="text-xs text-text-tertiary mt-0.5">{jsonShape.description}</p>}
      </div>
      <pre className="p-4 bg-surface-secondary overflow-x-auto">
        <code className="text-sm font-mono text-text-primary leading-relaxed">{jsonExample}</code>
      </pre>
    </div>
  );
}
