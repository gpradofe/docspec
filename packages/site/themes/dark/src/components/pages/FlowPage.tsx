            ...
            return (
              <div key={step.id} className="p-4 rounded-lg border border-border">
                <div className="flex items-center gap-2 mb-1">
                  <span className="w-6 h-6 rounded-full bg-primary-50 text-primary-500 flex items-center justify-center text-xs font-bold">
                    {i + 1}
                  </span>
                ...
                <div className="ml-8">
                  {step.actor && (
                    <div className="text-sm mb-1">
                      ...
                      {actorUrl ? (
                        <a href={`/${actorUrl}`} className="text-primary-500 hover:underline font-mono">
                          {step.actor}
                        </a>
                      ) : (
                  ...
                  <div className="flex gap-4 text-xs text-text-tertiary">
                    ...
                    {step.retryTarget && (
                      <span className="text-error">Retries &rarr; {step.retryTarget}</span>
                    )}
                  </div>
                      ...
                      <div className="mt-1 space-y-1">
                        {step.dataStoreOps.map((op, j) => (
                          <div key={j} className="flex items-center gap-2">
                            {op.store && <code className="font-mono text-text-secondary">{op.store}</code>}
                            {op.operation && <Badge>{op.operation}</Badge>}
                            {op.tables && op.tables.length > 0 && (
                              <span className="text-text-tertiary">({op.tables.join(", ")})</span>
                            )}
                            {op.transactional && <Badge variant="warning">txn</Badge>}
                            {op.cascading && <Badge variant="info">cascade</Badge>}
                          </div>
                        ))}
                      </div>
                    </div>
